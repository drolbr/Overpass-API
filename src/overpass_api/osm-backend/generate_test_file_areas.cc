...
{
  std::cout<<"  <node id=\""<<id<<"\" lat=\""
      <<std::fixed<<std::setprecision(7)<<lat<<"\" lon=\""<<lon<<"\"/>\n";
}
...
void create_way
  (uint id, const std::vector< uint >& refs,
   const std::vector< std::pair< std::string, std::string > >& tags)
{
  std::cout<<"  <way id=\""<<id<<"\">\n";
...
void create_way_test_rectangle
    (uint id, uint node_ref, const std::vector< std::pair< std::string, std::string > >& tags)
{
...
void create_triangle_test_pattern_ways(uint id, uint node_ref, double scale)
{
  ...
  {
    std::vector< std::pair< std::string, std::string > > tags;
    std::ostringstream v_dr, v_scale;
    v_dr<<i;
    tags.push_back(std::make_pair< std::string, std::string >("triangle", v_dr.str()));
    v_scale<<std::fixed<<std::setprecision(7)<<scale;
    tags.push_back(std::make_pair< std::string, std::string >("scale", v_scale.str()));
    create_way(id + i - 1,
...
{
  std::vector< std::pair< std::string, std::string > > tags;
  std::ostringstream v_scale;
  v_scale<<std::fixed<<std::setprecision(7)<<scale;
  tags.push_back(std::make_pair< std::string, std::string >("scale", v_scale.str()));

  tags.push_back(std::make_pair< std::string, std::string >("shapes", "1"));
  ...
  tags.push_back(std::make_pair< std::string, std::string >("shapes", "2"));
...
{
  std::vector< std::pair< std::string, std::string > > tags;
  tags.push_back(std::make_pair< std::string, std::string >("shapes", "3"));
  ...
  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("inner_point", "middle"));

  create_way(id + 1, V< uint >(node_ref + 121)(node_ref + 125)(node_ref + 129), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("inner_point", "first"));

  create_way(id + 2, V< uint >(node_ref + 125)(node_ref + 129)(node_ref + 133), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("inner_point", "last"));

  create_way(id + 3, V< uint >(node_ref + 121)(node_ref + 122)(node_ref + 125), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("inner_point", "total"));

  create_way(id + 4, V< uint >(node_ref + 124)(node_ref + 125)(node_ref + 126), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("touch_point", "middle"));

  create_way(id + 11, V< uint >(node_ref + 242)(node_ref + 243)(node_ref + 244), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("touch_point", "first"));

  create_way(id + 12, V< uint >(node_ref + 205)(node_ref + 246)(node_ref + 285), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("touch_point", "last"));

  create_way(id + 13, V< uint >(node_ref + 284)(node_ref + 281)(node_ref + 43), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("touch_point", "multiple_1"));

  create_way(id + 14, V< uint >(node_ref + 4)(node_ref + 45)(node_ref + 47)(node_ref + 8), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("touch_point", "multiple_2"));

  create_way(id + 15, V< uint >(node_ref + 201)(node_ref + 243)(node_ref + 208)(node_ref + 209), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("intersect", "middle_segment"));

  create_way(id + 21, V< uint >(node_ref + 81)(node_ref + 122)(node_ref + 88)(node_ref + 129), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("intersect", "first_segment"));

  create_way(id + 22, V< uint >(node_ref + 4)(node_ref + 244)(node_ref + 284), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("intersect", "last_segment"));

  create_way(id + 23, V< uint >(node_ref + 1)(node_ref + 2)(node_ref + 248), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("intersect", "single_segment"));

  create_way(id + 24, V< uint >(node_ref + 41)(node_ref + 49), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("intersect", "lat_segment"));

  create_way(id + 25, V< uint >(node_ref + 121)(node_ref + 129), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("intersect", "lon_segment"));

  create_way(id + 26, V< uint >(node_ref + 7)(node_ref + 287), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("intersect", "lat_boundary"));

  create_way(id + 27, V< uint >(node_ref + 204)(node_ref + 207), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("intersect", "lon_boundary"));

  create_way(id + 28, V< uint >(node_ref + 43)(node_ref + 203), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("through", "first_segment"));

  create_way(id + 31, V< uint >(node_ref + 43)(node_ref + 85)(node_ref + 6), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("through", "middle_segment"));

  create_way(id + 32, V< uint >(node_ref + 6)(node_ref + 85)(node_ref + 208)(node_ref + 248), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("through", "last_segment"));

  create_way(id + 33, V< uint >(node_ref + 281)(node_ref + 243)(node_ref + 85), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("through", "single_segment"));

  create_way(id + 34, V< uint >(node_ref + 243)(node_ref + 85), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("touch_segment", "multiple_segments"));
...
{
  std::vector< std::pair< std::string, std::string > > tags;
  tags.push_back(std::make_pair< std::string, std::string >("nodes", "outside"));

  create_relation(id + 1, V< uint >(node_ref + 245), V< uint >(0), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("nodes", "inside"));

  create_relation(id + 2, V< uint >(node_ref + 165), V< uint >(0), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("ways", "outside"));

  create_relation(id + 11, V< uint >(way_ref + 11), V< uint >(1), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("ways", "inside"));

  create_relation(id + 12, V< uint >(way_ref + 31), V< uint >(1), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("ways", "touch"));

  create_relation(id + 13, V< uint >(way_ref + 41), V< uint >(1), tags);

  tags.pop_back();
  tags.push_back(std::make_pair< std::string, std::string >("multiple", "last_inside"));
...
void create_multpolys_test_pattern_relations(uint id, uint way_ref, double scale)
{
  ...
  tags.push_back(std::make_pair< std::string, std::string >("multpoly", "1"));
  ...
  tags.push_back(std::make_pair< std::string, std::string >("multpoly", "2"));
  create_relation(id + 1, V< uint >(way_ref + 11)(way_ref + 13)(way_ref + 14),
      V< uint >(1)(1)(1), tags);
  tags.pop_back();

  tags.push_back(std::make_pair< std::string, std::string >("multpoly", "3"));
  create_relation(id + 2, V< uint >(way_ref + 12)(way_ref + 13)(way_ref + 14),
      V< uint >(1)(1)(1), tags);
  tags.pop_back();

  tags.push_back(std::make_pair< std::string, std::string >("multpoly", "4"));
