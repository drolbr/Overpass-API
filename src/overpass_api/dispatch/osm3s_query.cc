...
int main(int argc, char *argv[])
{
  ...
  while (argpos < argc)
  {
    ...
    else if (!(strcmp(argv[argpos], "--dump-pretty-std::map-ql")))
      debug_level = parser_dump_pretty_map_ql;
    else if (!(strcmp(argv[argpos], "--dump-compact-std::map-ql")))
      debug_level = parser_dump_compact_map_ql;
    else if (!(strcmp(argv[argpos], "--dump-bbox-std::map-ql")))
    ...
    else
    {
      ...
      "  --dump-pretty-std::map-ql: Don't execute the query but only dump the query in pretty QL format.\n"
      "  --dump-compact-std::map-ql: Don't execute the query but only dump the query in compact QL format.\n"
      "  --dump-bbox-std::map-ql: Don't execute the query but only dump the query in a suitable form\n"
