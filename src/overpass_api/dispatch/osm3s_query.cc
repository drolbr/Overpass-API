...
int main(int argc, char *argv[])
{
  ...
  while (argpos < argc)
  {
    if (!(strncmp(argv[argpos], "--db-dir=", 9)))
    {
      ...
      if (db_dir.empty())
      {
        std::cerr<<"Please provide a directory name after --db-dir=\n";
        return 0;
      }
      if (db_dir[db_dir.size()-1] != '/')
	db_dir += '/';
    }
  ...
  try
  {
    ...
    if (clone_db_dir != "")
    {
      ...
      Dispatcher_Stub dispatcher(
          db_dir, error_output, "-- clone database --",
          area_level, 24*60*60, 1024*1024*1024, global_settings);
