...
{
public:
  Parsed_Query() : output_handler(0), global_bbox_limitation(Bbox::invalid) {}
  ~Parsed_Query() { delete output_handler; }
  
  Output_Handler* get_output_handler() { return output_handler; }
  void set_output_handler(Output_Handler* handler);
  void set_global_bbox(const Bbox& bbox) { global_bbox_limitation = bbox; }
  
  void trigger_print_bounds();
  const Bbox& get_global_bbox_limitation() const { return global_bbox_limitation; }
...
inline void Parsed_Query::trigger_print_bounds()
{
