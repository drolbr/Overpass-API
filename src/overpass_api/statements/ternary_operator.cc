    ...
    Statement::Factory& stmt_factory, Error_Output* error_output)
{
  if (result)
  {
    ...
    Statement* condition = stmt_factory.create_evaluator(tree_it.lhs(), tree_context);
    if (condition)
    ...
    Statement* lhs = stmt_factory.create_evaluator(tree_it.rhs().lhs(), tree_context);
    if (lhs)
    ...
    Statement* rhs = stmt_factory.create_evaluator(tree_it.rhs().rhs(), tree_context);
    if (rhs)
...
Eval_Task* Ternary_Evaluator::get_task(Prepare_Task_Context& context)
{
  Eval_Task* cond_task = condition ? condition->get_task(context) : 0;
