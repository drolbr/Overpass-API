...
{
  nodes = relation_node_members(&query, rman, rels, ranges, ids, ids.empty() ? true : invert_ids);
}
                   ...
                   std::map< Uint32_Index, std::vector< Attic< Node_Skeleton > > >& attic_nodes)
{
  swap_components(relation_node_members
      (&query, rman, rels, attic_rels, ranges, ids, ids.empty() ? true : invert_ids), nodes, attic_nodes);
}
...
void collect_nodes(const Statement& query, Resource_Manager& rman,
                   ...
                   uint32 role_id)
{
  nodes = relation_node_members(&query, rman, rels, ranges, ids, ids.empty() ? true : invert_ids, &role_id);
}
...
{
  swap_components(relation_node_members
      (&query, rman, rels, attic_rels, ranges, ids, ids.empty() ? true : invert_ids, &role_id), nodes, attic_nodes);
}
                   ...
                   std::map< Uint32_Index, std::vector< Attic< Node_Skeleton > > >& attic_nodes)
{
  ...
  {
    if (ids.empty())
      swap_components(way_members(&query, rman, ways, attic_ways, pos, 0, ids, true), nodes, attic_nodes);
    else
      swap_components(way_members(&query, rman, ways, attic_ways, pos, 0, ids, invert_ids), nodes, attic_nodes);
  }
  else
  {
    if (ids.empty())
      swap_components(way_members(&query, rman, ways, attic_ways, pos, &ranges, ids, true), nodes, attic_nodes);
    else
      swap_components(way_members(
          &query, rman, ways, attic_ways, pos, invert_ids ? 0 : &ranges, ids, invert_ids), nodes, attic_nodes);
  }
}
		    ...
		    std::map< Uint31_Index, std::vector< Relation_Skeleton > >& result)
{
  ...
  while (true)
  {
    result = relation_relation_members(query, rman, source, 0, {}, true);
    sort_second(source);
    sort_second(result);
                    ...
                    std::map< Uint31_Index, std::vector< Attic< Relation_Skeleton > > >& attic_result)
{
  ...
  while (true)
  {
    std::pair< std::map< Uint31_Index, std::vector< Relation_Skeleton > >,
        ...
        = relation_relation_members(query, rman, source, attic_source, 0, {}, true);
    sort_second(source);
...
bool Recurse_Constraint::get_data
    (const Statement& query, Resource_Manager& rman, Set& into,
     const std::set< std::pair< Uint32_Index, Uint32_Index > >& ranges_,
     const std::vector< Node::Id_Type >& ids,
     bool invert_ids)
{
  const Set* input = rman.get_set(stmt->get_input());
  if (!input)
    return true;
  Ranges< Uint32_Index > ranges(ranges_.empty()
  ...
  if (rman.get_desired_timestamp() == NOW)
  {
    ...
    if (stmt->get_type() == RECURSE_RELATION_NODE || stmt->get_type() == RECURSE_RELATION_NWR
      ...
      ::collect_nodes(query, rman, input->ways, input->attic_ways, stmt->get_pos(), ranges_, ids, invert_ids,
    ...
    else if (stmt->get_type() == RECURSE_DOWN)
    {
      ...
      rel_ways = relation_way_members(&query, rman, input->relations, std::set< std::pair< Uint31_Index, Uint31_Index > >{{ Uint31_Index(0u), Uint31_Index(0x7fffffffu) }}, {}, true);
      ::collect_nodes(query, rman, rel_ways, rel_attic_ways, 0, ranges_, ids, invert_ids,
          into.nodes, into.attic_nodes);
    ...
    else if (stmt->get_type() == RECURSE_DOWN_REL)
    {
      ...
      rel_ways = relation_way_members(&query, rman, rel_rels, std::set< std::pair< Uint31_Index, Uint31_Index > >{{ Uint31_Index(0u), Uint31_Index(0x7fffffffu) }}, {}, true);
      ::collect_nodes(query, rman, rel_ways, rel_attic_ways, 0, ranges_, ids, invert_ids,
          into.nodes, into.attic_nodes);
      sort_second(into.nodes);
      sort_second(rel_nodes);
  ...
  else
  {
    ...
    if (stmt->get_type() == RECURSE_RELATION_NODE || stmt->get_type() == RECURSE_RELATION_NWR
      ...
      ::collect_nodes(query, rman, input->ways, input->attic_ways, stmt->get_pos(),
                      ranges_, ids, invert_ids, into.nodes, into.attic_nodes);
    else if (stmt->get_type() == RECURSE_DOWN)
    {
      std::map< Uint32_Index, std::vector< Node_Skeleton > > rel_nodes;
      ...
      std::pair< std::map< Uint31_Index, std::vector< Way_Skeleton > >,
          ...
          = relation_way_members(&query, rman, input->relations, input->attic_relations, std::set< std::pair< Uint31_Index, Uint31_Index > >{{ Uint31_Index(0u), Uint31_Index(0x7fffffffu) }}, {}, true);
    ...
    else if (stmt->get_type() == RECURSE_DOWN_REL)
    {
      ...
      std::pair< std::map< Uint31_Index, std::vector< Way_Skeleton > >,
          std::map< Uint31_Index, std::vector< Attic< Way_Skeleton > > > > rel_ways
          = relation_way_members(&query, rman, rel_rels, attic_rel_rels, std::set< std::pair< Uint31_Index, Uint31_Index > >{{ Uint31_Index(0u), Uint31_Index(0x7fffffffu) }}, {}, true);
      ::collect_nodes(query, rman, rel_ways.first, rel_ways.second, 0,
                      ranges_, ids, invert_ids, into.nodes, into.attic_nodes);
     ...
     bool invert_ids)
{
  ...
  if (rman.get_desired_timestamp() == NOW)
  {
    ...
    {
      if (ids.empty())
        collect_ways(query, rman, input->nodes, stmt->get_pos(), into.ways);
      else
    ...
    else if (stmt->get_type() == RECURSE_UP)
    {
      ...
      {
        if (ids.empty())
	  collect_ways(query, rman, input->nodes, 0, into.ways);
        else
	  collect_ways(query, rman, input->nodes, 0, into.ways,
		       ids, invert_ids);
      }
  ...
  else
  {
    ...
    if (stmt->get_type() == RECURSE_RELATION_WAY
          ...
          || (stmt->get_type() == RECURSE_RELATION_WR && type == QUERY_WAY))
      collect_ways(query, rman, input->relations, input->attic_relations,
                   ranges_, ids, ids.empty() || invert_ids, into.ways, into.attic_ways);
    else if (stmt->get_type() == RECURSE_RELATION_RELATION
          || (stmt->get_type() == RECURSE_RELATION_NWR && type == QUERY_RELATION)
          || (stmt->get_type() == RECURSE_RELATION_NR && type == QUERY_RELATION)
          || (stmt->get_type() == RECURSE_RELATION_WR && type == QUERY_RELATION))
    ...
    else if (stmt->get_type() == RECURSE_NODE_WAY)
    {
      if (ids.empty())
        collect_ways(query, rman,
                     input->nodes, input->attic_nodes, stmt->get_pos(), into.ways, into.attic_ways);
      else
        collect_ways(query, rman,
                     input->nodes, input->attic_nodes, stmt->get_pos(), into.ways, into.attic_ways,
                     ids, invert_ids);
    ...
    else if (stmt->get_type() == RECURSE_UP)
    {
      if (type == QUERY_WAY)
      {
        if (ids.empty())
          collect_ways(query, rman,
                       input->nodes, input->attic_nodes, 0, into.ways, into.attic_ways);
        else
          collect_ways(query, rman,
                       input->nodes, input->attic_nodes, 0, into.ways, into.attic_ways,
                       ids, invert_ids);
...
void Recurse_Constraint::filter(const Statement& query, Resource_Manager& rman, Set& into)
{
  ...
  if (stmt->get_type() == RECURSE_DOWN)
  {
    ...
    else
    {
      ...
      collect_ways(query, rman, input->relations, input->attic_relations,
          std::set< std::pair< Uint31_Index, Uint31_Index > >{{ Uint31_Index(0u), Uint31_Index(0x7fffffffu) }}, std::vector< Way::Id_Type >{}, true,
...
void Recurse_Statement::execute(Resource_Manager& rman)
{
  ...
  if (restrict_to_role)
  {
        ...
        || type == RECURSE_RELATION_NR)
    {
      if (rman.get_desired_timestamp() == NOW)
        ...
        swap_components(relation_relation_members(
                *this, rman, input_set->relations, input_set->attic_relations, 0, {}, true, &role_id),
            into.relations, into.attic_relations);
    }
        ...
        || type == RECURSE_RELATION_WR)
    {
      if (rman.get_desired_timestamp() == NOW)
        into.ways = relation_way_members(this, rman, input_set->relations,
            std::set< std::pair< Uint31_Index, Uint31_Index > >{{ Uint31_Index(0u), Uint31_Index(0x7fffffffu) }}, {}, true, &role_id);
      else
        swap_components(relation_way_members(
                this, rman, input_set->relations, input_set->attic_relations, std::set< std::pair< Uint31_Index, Uint31_Index > >{{ Uint31_Index(0u), Uint31_Index(0x7fffffffu) }}, {}, true, &role_id),
        ...
        || type == RECURSE_RELATION_NR)
    {
      if (rman.get_desired_timestamp() == NOW)
        into.nodes = relation_node_members(
            ...
            std::set< std::pair< Uint32_Index, Uint32_Index > >{{ Uint32_Index(0u), Uint32_Index(0x7fffffff) }}, {}, true, &role_id);
      else
        swap_components(relation_node_members(
  ...
  {
    if (rman.get_desired_timestamp() == NOW)
      into.relations = relation_relation_members(*this, rman, input_set->relations, 0, {}, true);
    else
      swap_components(relation_relation_members(*this, rman, input_set->relations, input_set->attic_relations, 0, {}, true),
          into.relations, into.attic_relations);
  }
  else if (type == RECURSE_RELATION_BACKWARDS)
  {
  ...
  {
    if (rman.get_desired_timestamp() == NOW)
      into.ways = relation_way_members(this, rman, input_set->relations, std::set< std::pair< Uint31_Index, Uint31_Index > >{{ Uint31_Index(0u), Uint31_Index(0x7fffffffu) }}, {}, true);
    else
      swap_components(relation_way_members(this, rman, input_set->relations, input_set->attic_relations, std::set< std::pair< Uint31_Index, Uint31_Index > >{{ Uint31_Index(0u), Uint31_Index(0x7fffffffu) }}, {}, true),
          into.ways, into.attic_ways);
  }
  else if (type == RECURSE_RELATION_NODE)
  {
    if (rman.get_desired_timestamp() == NOW)
      into.nodes = relation_node_members(this, rman, input_set->relations, std::set< std::pair< Uint32_Index, Uint32_Index > >{{ Uint32_Index(0u), Uint32_Index(0x7fffffff) }}, {}, true);
    else
      swap_components(relation_node_members(this, rman, input_set->relations, input_set->attic_relations, std::set< std::pair< Uint32_Index, Uint32_Index > >{{ Uint32_Index(0u), Uint32_Index(0x7fffffff) }}, {}, true),
          into.nodes, into.attic_nodes);
  }
  ...
  else if (type == RECURSE_DOWN_REL)
  {
    if (rman.get_desired_timestamp() == NOW)
    {
      ...
      std::map< Uint32_Index, std::vector< Node_Skeleton > > rel_nodes
          = relation_node_members(this, rman, into.relations, std::set< std::pair< Uint32_Index, Uint32_Index > >{{ Uint32_Index(0u), Uint32_Index(0x7fffffff) }}, {}, true);
      into.ways = relation_way_members(this, rman, into.relations, std::set< std::pair< Uint31_Index, Uint31_Index > >{{ Uint31_Index(0u), Uint31_Index(0x7fffffffu) }}, {}, true);
    ...
    else
    {
      relations_loop(*this, rman, input_set->relations, input_set->attic_relations,
                     into.relations, into.attic_relations);
      swap_components(relation_node_members(this, rman, into.relations, into.attic_relations, std::set< std::pair< Uint32_Index, Uint32_Index > >{{ Uint32_Index(0u), Uint32_Index(0x7fffffff) }}, {}, true),
          into.nodes, into.attic_nodes);
      swap_components(relation_way_members(this, rman, into.relations, into.attic_relations, std::set< std::pair< Uint31_Index, Uint31_Index > >{{ Uint31_Index(0u), Uint31_Index(0x7fffffffu) }}, {}, true),
          into.ways, into.attic_ways);
