...
void Output_CSV::write_payload_header(const std::string&, const std::string&, const std::string&)
{
  ...
  {
    for (auto it = data_printer.csv_settings.keyfields.begin();
        it != data_printer.csv_settings.keyfields.end(); ++it)
    {
      std::cout<<(it->second ? "@" : "")<<escape_csv(it->first);
      if (it + 1 != data_printer.csv_settings.keyfields.end())
        std::cout<<data_printer.csv_settings.separator;
    }
    std::cout<<'\n';
  }
}
...
std::string Output_CSV::dump_config() const
{
  ...
  for (auto it = data_printer.csv_settings.keyfields.begin(); it != data_printer.csv_settings.keyfields.end(); ++it)
  {
    if (it != data_printer.csv_settings.keyfields.begin())
      result += ",";
  ...
  if (data_printer.csv_settings.separator != "\t")
    result += std::string(";") + (data_printer.csv_settings.with_headerline ? "true" : "false") + ";\""
        + escape_cstr(data_printer.csv_settings.separator) + "\"";
  else if (!data_printer.csv_settings.with_headerline)
    result += ";false";
...
void Output_CSV::Data_Printer::print_item(const Derived_Skeleton& skel,
