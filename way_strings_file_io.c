...
const string WAY_STRING_DATA = (string)DATADIR + "way_strings.dat";
const string WAY_STRING_IDX = (string)DATADIR + "way_strings.idx";
...
void way_tag_split_and_index(uint& current_run, vector< uint32 >& split_idx, uint32*& block_of_id)
{
  ...
  int dest_fd = open64(WAY_STRING_DATA.c_str(), O_WRONLY|O_CREAT|O_TRUNC,
		       S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
  close(dest_fd);
  
  dest_fd = open64(WAY_STRING_DATA.c_str(), O_WRONLY|O_CREAT, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
  if (dest_fd < 0)
    throw File_Error(errno, WAY_STRING_DATA, "way_tag_split_and_index:2");
  
  int dest_idx_fd = open64(WAY_STRING_IDX.c_str(), O_WRONLY|O_CREAT|O_TRUNC,
			   S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
  close(dest_idx_fd);
  
  dest_idx_fd = open64(WAY_STRING_IDX.c_str(), O_WRONLY|O_CREAT, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
  if (dest_fd < 0)
    throw File_Error(errno, WAY_STRING_IDX, "way_tag_split_and_index:3");
...
{
  int dest_fd = open64(WAY_TAG_ID_STATS.c_str(), O_WRONLY|O_CREAT|O_TRUNC,
		       S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
  close(dest_fd);
  
  dest_fd = open64(WAY_TAG_ID_STATS.c_str(), O_WRONLY|O_CREAT, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
  if (dest_fd < 0)
    throw File_Error(errno, WAY_TAG_ID_STATS, "way_tag_id_statistics:1");
...
struct Way_String_Cache
{
  ...
  private:
    static void init()
    {
      ...
      int string_idx_fd = open64(WAY_STRING_IDX.c_str(), O_RDONLY);
      if (string_idx_fd < 0)
	throw File_Error(errno, WAY_STRING_IDX, "Way_String_Cache.init():1");
			       ...
			       vector< uint32 >& local_id_idx, vector< uint32 >& spatial_boundaries_)
{
  ...
  int string_idx_fd = open64(WAY_STRING_IDX.c_str(), O_WRONLY|O_APPEND);
  if (string_idx_fd < 0)
    throw File_Error(errno, WAY_STRING_IDX, "Way_String_Cache.init():1");
  ...
  string_idx_fd = open64(WAY_STRING_IDX.c_str(), O_WRONLY);
  if (string_idx_fd < 0)
    throw File_Error(errno, WAY_STRING_IDX, "Way_String_Cache.init():1");
...
void way_tag_id_statistics_remake()
{
  ...
  dest_fd = open64(WAY_TAG_ID_STATS.c_str(), O_WRONLY|O_CREAT, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
  if (dest_fd < 0)
    throw File_Error(errno, WAY_TAG_ID_STATS, "way_tag_id_statistics_remake:1");
     ...
     set< uint32 >& string_ids_local, set< uint32 >& string_idxs_local)
{
  ...
  int string_fd = open64(WAY_STRING_DATA.c_str(), O_RDONLY);
  if (string_fd < 0)
    throw File_Error(errno, WAY_STRING_DATA, "select_kv_to_ids:1");
     ...
     map< uint32, pair< string, string > >& kvs)
{
  ...
  int string_fd = open64(WAY_STRING_DATA.c_str(), O_RDONLY);
  if (string_fd < 0)
    throw File_Error(errno, WAY_STRING_DATA, "select_ids_to_kvs:1");
