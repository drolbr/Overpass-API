...
shortcuts = cgi-bin/download cgi-bin/poi cgi-bin/elems cgi-bin/nodes-csv-lat-lon-name cgi-bin/nodes-csv
...
cgi-bin/nodes-csv: utils/nodes-csv.sh
	cp $< $@
	chmod 755 $@

cgi-bin/nodes-csv-lat-lon-name: utils/nodes-csv-lat-lon-name.c expat_justparse_interface.h expat_justparse_interface.o
	g++ -lexpat -o $@ -O3 -Wall $^
